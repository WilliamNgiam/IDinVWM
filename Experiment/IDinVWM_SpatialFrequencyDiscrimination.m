...
calibration.allChanges = mod(randperm(experiment.nCalibrationTrials),2)+1;          % Determines which pair will have different Gabors
...
calibration.FirstSpatialFrequency_cpd = (rand(1,experiment.nCalibrationTrials)*9+1); % Spatial frequency in cycles per degree (Using the Robson (1966) paper CSF to determine appropriate range - About 1 cpd to 10 cpd)
calibration.SecondSpatialFrequency_cpd = (rand(1,experiment.nCalibrationTrials)*9+1); % The input for the DrawTexture is in cycles per pixel

calibration.FirstSpatialFrequency_cpp = calibration.FirstSpatialFrequency_cpd/equipment.ppd; % First spatial frequency in cycles per pixel
calibration.SecondSpatialFrequency_cpp = calibration.SecondSpatialFrequency_cpd/equipment.ppd; % Second spatial frequency in cycles per pixel
...
for thisCalibrationTrial = 1:experiment.nCalibrationTrials
    ...
    whichOne = calibration.allChanges(thisCalibrationTrial);
    ...
    if whichOne == 1 % First interval is different
        ...
        if calibration.allChangeDirections(thisCalibrationTrial) == 1
            ...
            Screen('DrawTexture', ptbWindow, gaborid, [], itemRects(:,stimulus.nItems), stimulus.orientation_deg, [], [], [], [], kPsychDontDoRotation, [calibration.FirstPhase(thisCalibrationTrial), calibration.FirstSpatialFrequency_cpp(thisCalibrationTrial)+thisSpatialFrequencyDifference, stimulus.sc_pix, stimulus.contrast, stimulus.aspectratio, 0, 0, 0]);
    ...
    allCalibrationResponses(thisCalibrationTrial) = pressedKey;
